...
const rectyp_tests = [
    RecordTypeTest(
        DirectAccess, "fixed length (direct access)",
        "gendatadirect", "DA", ""
    ),
    RecordTypeTest(
        RECMRK4Bwssr, "markers of 4-byte with short subrecords",
        "gendataseq", "4Bshort", "-fmax-subrecord-length=$(maxsubreclen)"
    ),
    RecordTypeTest(
        RECMRK4B, "markers of 4-byte with default subrecords",
        "gendataseq", "4Bdef", ""
    ),
    RecordTypeTest(
        RECMRK8B, "markers of 8-byte without subrecords",
        "gendataseq", "8B", "-frecord-marker=8"
    ),
    RecordTypeTest(
        RECMRK4Bwosr, "markers of 4-byte without subrecords",
        "gendataseq", "4Bdumb", ""
    ),
]
...
function gendata(target, tag, fflags)
    ...
    return `codegen/$(target)_$(tag).x data_$(tag).bin` |> run
end
...
function writedata(f::FortranFile{DirectAccess}, data)
    ...
    return @test nwritten == sizeof(varstr)
end
